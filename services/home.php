...
<script type="text/javascript">var scrollWatcher=createScrollWatcher();Navbar.init();const header=document.querySelector("header#home"),navbarToggler=document.querySelector("#navbarToggler"),contactForm=document.forms.contact,modalContact=document.querySelector("#modalContact"),alertContact=document.querySelector("#alertContact"),sections=document.querySelectorAll("[data-section-register]"),modalPlantation=document.getElementById("modalPlantation");Array.from(sections).forEach(e=>{SectionRegister.push(e.getAttribute("data-section-name"),e)}),loadImgHeaderHero(header);let activeSection=SectionRegister.getActiveRange();Navbar.setActivedMenu(activeSection);const imgList=lazyLoadImgAnalyzerForHomePage();loadImg(imgList,!0),modalPlantation.addEventListener("show.bs.modal",function(e){let t=modalPlantation.querySelectorAll("img[data-loaded='false']");loadImg([...t],!0)}),navbarToggler.addEventListener("click",()=>{Navbar.toggleCollapse()}),contactForm.btnSubmit.addEventListener("click",()=>{if(!contactForm.checkValidity())return void contactForm.classList.add("was-validated");let e=new FormData(contactForm),t=new XMLHttpRequest;t.open("POST","<?=WEB_DOMAIN?>mail",!0),t.onload=function(){let e=this.responseText;try{(e=JSON.parse(e)).status?(bootstrap.Modal.getInstance(modalContact).hide(),contactForm.messege.value=""):(bsAlert(alertContact,"danger","<b>Failed</b> to send your messeges! Try other option, please."),document.querySelector("#modalContact .modal-body").scrollTop=alertContact.offsetTop)}catch(e){console.log(e.messege)}},t.send(e),contactForm.classList.remove("was-validated")}),contactForm.btnReset.addEventListener("click",()=>{contactForm.reset(),contactForm.classList.remove("was-validated")}),window.onscroll=(()=>{Navbar.scrolledStyle(),scrollWatcher.setKey(),activeSection!=SectionRegister.getActiveRange()&&(activeSection=SectionRegister.getActiveRange(),Navbar.setActivedMenu(activeSection))}),AOS.init();</script>
</body>
</html>